...
import (
	...
	"product_srv/models"
	product_srv "product_srv/proto/seckill"
	"product_srv/utils"
	"time"
)
...
func (s *SecKills) SecKillList(ctx context.Context, in *product_srv.SecKillsRequest, out *product_srv.SecKillsResponse) error {
	...
	seckills_rep := []*product_srv.SecKill{}

	for _, seckill := range seckills {
		seckill_rep := product_srv.SecKill{}
...
func (s *SecKills) GetProducts(ctx context.Context, in *product_srv.ProductRequest, out *product_srv.ProductResponse) error {

	products := []models.Products{}

	result := data_source.Db.Find(&products)

	products_rep := []*product_srv.Product{}
	...
	for _, product := range products {
		product_rep := product_srv.Product{}
...
func (s *SecKills) SecKillAdd(ctx context.Context, in *product_srv.SecKill, out *product_srv.SecKillResponse) error {
...
func (s *SecKills) SecKillDel(ctx context.Context, in *product_srv.SecKillDelRequest, out *product_srv.SecKillResponse) error {
	id := in.Id
	// 删除数据库数据操作
	seckill := models.SecKills{
		Id: int(id),
...
func (s *SecKills) SecKillToEdit(ctx context.Context, in *product_srv.SecKillDelRequest, out *product_srv.SecKillToEditResponse) error {
	id := in.Id
...
func (s *SecKills) SecKillDoEdit(ctx context.Context, in *product_srv.SecKill, out *product_srv.SecKillResponse) error {
	id := in.Id
	name := in.Name
	price := in.Price
	num := in.Num
...
func (s *SecKills) FrontSecKillList(ctx context.Context, in *product_srv.FrontSecKillRequest, out *product_srv.FrontSecKillResponse) error {
	...
	*/
	...
	//result := data_source.Db.Where("status = ?", 0).Limit(pagesize).Offset(offsetNum).Find(&seckills)
	if result.Error != nil {
		out.Code = 500
		out.Msg = "查询不到数据"
...
func (s *SecKills) FrontSecKillDetail(ctx context.Context, in *product_srv.SecKillDelRequest, out *product_srv.FrongSecKillDetailResponse) error {
