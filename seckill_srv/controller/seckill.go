...
import (
	"context"
	seckill_srv "seckill_srv/proto/seckill"

	"fmt"
	"seckill_srv/data_source"
	"seckill_srv/models"
	"seckill_srv/redis_lib"
	"seckill_srv/utils"
	"time"
...
func (s *SecKill) FrontSecKill(ctx context.Context, in *seckill_srv.SecKillRequest, out *seckill_srv.SecKillResponse) error {
	id := in.Id
...
func init() {
	...
	conn, err := amqp.Dial("amqp://xlf:123456@81.68.142.83:5672")
	fmt.Println(err)
	defer conn.Close()
	...
	deliveries, err := ch.Consume("web.order_queue", "order_consumer", false, false, false, false, nil)
